...
class Tracker():
    ...
    def maximum_loss_examples(self, val_inputs, val_outputs, val_labels, val_preds, epoch, path_mean, path_std):
        ...
        self.writer.add_figure('UnNormalisedErrors/1stLarge', self.patch_plot(np.transpose(unnorm(ordered_input[0,:,:,:].cpu()), (2,1,0)), ordered_labels[0], ordered_preds[0], ordered_losses[0]), global_step = epoch)
        self.writer.add_figure('UnNormalisedErrors/2ndLarge', self.patch_plot(np.transpose(unnorm(ordered_input[1,:,:,:].cpu()), (2,1,0)), ordered_labels[1], ordered_preds[1], ordered_losses[1]), global_step = epoch)
        self.writer.add_figure('UnNormalisedErrors/3rdLarge', self.patch_plot(np.transpose(unnorm(ordered_input[2,:,:,:].cpu()), (2,1,0)), ordered_labels[2], ordered_preds[2], ordered_losses[2]), global_step = epoch)

        self.writer.add_figure('UnNormalisedErrors/1stSmallest', self.patch_plot(np.transpose(unnorm(ordered_input[-1,:,:,:].cpu()), (2,1,0)), ordered_labels[-1], ordered_preds[-1], ordered_losses[-1]), global_step = epoch)
        self.writer.add_figure('UnNormalisedErrors/2ndSmallest', self.patch_plot(np.transpose(unnorm(ordered_input[-2,:,:,:].cpu()), (2,1,0)), ordered_labels[-2], ordered_preds[-2], ordered_losses[-2]), global_step = epoch)
        self.writer.add_figure('UnNormalisedErrors/3rdSmallest', self.patch_plot(np.transpose(unnorm(ordered_input[-3,:,:,:].cpu()), (2,1,0)), ordered_labels[-3], ordered_preds[-3], ordered_losses[-3]), global_step = epoch)
